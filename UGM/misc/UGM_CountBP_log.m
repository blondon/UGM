function [msg_i,msg_o] = UGM_CountBP_log(nodePot,edgePot,nodeCount,edgeCount,edgeStruct,momentum,convTol,maximize)
...
for e = 1:nEdges
	...
	edgePot(1:nStates(n1),1:nStates(n2),e) = ...
        log(edgePot(1:nStates(n1),1:nStates(n2),e)).*(1/edgeCount(e));
	% Init exponents
...
for i = 1:edgeStruct.maxIter
	...
	for e = 1:nEdges
		...
		if maximize
			...
			% Sum-product
			tmp_i(1:nStates(n1),e) = logsumexp(bsxfun(@plus, pot, msg_o(1:nStates(n2),e+nEdges)'))';
			tmp_i(1:nStates(n2),e+nEdges) = logsumexp(bsxfun(@plus, pot', msg_o(1:nStates(n2),e+nEdges)'))';
		end
